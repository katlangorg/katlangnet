...
| one sequence value → its slots          | `spread : Val → Option Supply`   |
...
| open one grouped value for a deconstruction receiver / sequence builtin | `normalizeSupply : Supply → Supply` |
...
| assignment deconstruction binding (opens a single sequence) | `bindDeconstruct : List Pat → Supply → Option Env` |

The headline laws in `CoreArityAlgebraProofs.lean` (all proved, no `sorry`):

* `spread (Val.seq xs) = some xs` — spread undoes raw sequence construction; `Val.seq` is a section of `spread`.
...
* rest / variadic capture use `capture xs = normalize (Val.seq xs)` — no second
  ...
  `spread (capture [1]) = none` while `spread (Val.seq [1]) = some [1]`.)

`Val.seq` is deliberately the *raw* constructor: the algebra needs it to state
section laws such as `spread (Val.seq xs) = some xs` and to model
...
`normalize ∘ Val.seq`, **not** the bare constructor: only `Val.seq` is the exact section of `spread`
(`spread (Val.seq [1]) = some [1]`, whereas `spread (capture [1]) = none`). KatLang
...
* `bindDeconstruct` is `bindPats ∘ normalizeSupply`: assignment deconstruction is an
  unpacking receiver (Python-style), so a single sequence-valued right-hand side is
  ...
  `A` as one argument). `normalizeSupply` is the shared singleton-boundary opening:
  it is used by `bindDeconstruct` and by sequence-builtin collection binding
  (`count(A)`, where the one grouped value IS the collection), but never by `bindArgs`.
  (In `KatLang.lean` these are two code paths — builtin item-supply normalization
  ...
  the core model unifies them as the single `normalizeSupply`.)
