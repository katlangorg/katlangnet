...
| call parameter binding (binds the supply as it is — values stay values) | `bindArgs : List Pat → Supply → Option Env` (the shared `bindPats` itself) |
| callback binding (the ordinary call binder) | `bindCallback : List Pat → List Val → Option Env` (`bindArgs`) |
...
| `openLoneStructure` | supply → supply | deconstruction-specific supply preparation (opens a lone sequence OR list) |

## The receiver split

The two binding receivers are the same front/collecting/back kernel (`bindPats`)
with different supply preparation — and the call receiver has none:

```lean
def bindArgs        (ps : List Pat) (xs : Supply)     := bindPats ps xs
def bindCallback    (ps : List Pat) (args : List Val) := bindArgs ps args
def bindDeconstruct (ps : List Pat) (xs : Supply)     := bindPats ps (openLoneStructure xs)
```

**Values stay values** (September 2026; this supersedes the collector
supply-boundary law and its `CallSupply` / `Origin` / `collectorSupply` /
`fuseCollectorSegment` call-supply preparation, which are deleted). A call's
supply holds ONE item per non-spread argument — whatever its value: an atom, a
sequence, a list, `()`, `[]` — plus the immediate items of each explicitly
spread operand (`items`), and `bindArgs` binds that supply AS IT IS: fixed
captures take one item unchanged, and a collector collects exactly its
allocated items. So `Coll((1, 2))` binds `[(1, 2)]`, `Coll(())` binds `[()]`,
`Coll(((1, 2), 3))` binds `[((1, 2), 3)]`, `Coll((1, 2), 3)` binds
`[(1, 2), 3]`, `Coll([1, 2])` binds `[[1, 2]]`, and only the spread supplies
`Coll((1, 2)*)` and `Coll([1, 2]*)` bind `[1, 2]` (`Coll([(1, 2)]*)` binds
`[(1, 2)]`: a spread-produced item is never reopened). Fixed positions are
allocated first and the collector collects the rest: `F(x, *rest)` on
`F(1, (2, 3))` binds `rest = [(2, 3)]`, on `F(1, (2, 3), 4)` it binds
`rest = [(2, 3), 4]`, and a lone sequence against a fixed position stays one
value (`Add((1, 2))` is an arity error, `Id((1, 2))` binds the pair). The
collector laws are `collector_lone_sequence_is_one_item`,
`collector_lone_list_is_one_item`, `collector_lone_empty_values_are_one_item`,
`collector_many_items_are_exact`, `collector_spread_supply_is_collected`,
`collector_spread_opening_is_one_level`, the
`collector_*_segment_*_is_exact` allocation instances,
`collector_lone_middle_is_exact_and_spread_opens`,
`collector_lone_argument_is_one_item` / `collector_spread_argument_is_its_items`
(stated over `bindArgs`), and `forwarding_resupplies_the_collected_supply`
(`spread(collect(S)) = S` through a forwarding collector).

**The callback law** is the same binder: a callback operation supplies each
value it passes — a `map`/`filter` element, a `reduce` element and accumulator
— as ONE ordinary argument, so `map(xs, F)` binds `bindCallback ps [e]`,
exactly the direct call `F(e)` (`callback_is_the_ordinary_call`,
`callback_element_is_one_ordinary_argument`). A structured element is one
argument, so a two-fixed flat callee rejects it
(`callback_two_fixed_rejects_structured_element`: `map([(1, 2)], Add)` fails
like `Add((1, 2))`), and the reducer's accumulator is one argument beside the
element (`callback_accumulator_is_one_ordinary_argument`: `Acc(x, *acc)`
collects `[acc]`).

Assignment deconstruction instead opens ONE lone sequence or lone list of the
whole supply (`loneStructure` characterizes exactly that supply shape). The
receiver theorems split the whole story on that predicate:

* `receivers_agree_outside_lone_structure` — on a supply with
...
* `deconstruct_singleton_eq_args_items` — on a single-value supply the
  ...
  value's immediate item view (the supply of the explicit spread `F(v*)`);
* `receivers_never_same_on_lone_structure`, with its
  `receivers_never_same_on_lone_seq` / `receivers_never_same_on_lone_list`
  instances — on a lone SEQUENCE or LIST the call receiver (which keeps the one
  supplied value) and the deconstruction receiver (which opens it) NEVER both
  succeed with the same environment, for ANY pattern list, the lone collecting
  pattern included. Shared *success* is the strongest correct claim — both
  receivers can fail identically (two fixed names vs a one-item payload), so
  plain Option inequality would be false;
* `lone_collecting_disagrees_on_lone_structure` — the lone-collecting instance,
  where both modes DO succeed, so even the Option values differ: a call
  collector collects the one argument (`rest = [A]`), while deconstruction
  opens it.

The grouped/spread relation at a single collecting parameter follows:
`variadic_collect_distinguishes_spread_seq` and
`variadic_collect_distinguishes_spread_list` (`F(A)` and `F(A*)` differ for
every stored structure, a sequence and a list alike), with the exact bound
values in `variadic_collect_value_grouped` (`rest = [A]`) and
`variadic_collect_value_spread` (`rest = [a₁, …]`).
...
deconstruction collecting bindings (`first, *mid, last = …`) and collecting parameters
...
establish segment collection for every single-collecting parameter list at once: for
...
over `bindPats`, the shared kernel, and `bindArgs` IS that kernel, so the
collector's `mid` is always exactly the supplied items
(`bindArgs_lone_collecting`): one item per non-spread argument, a lone
sequence included (`collector_lone_argument_is_one_item`).
...
* `collect` is exact: a collecting parameter that collects exactly one item binds the
...
* `bindArgs` consumes the call's supplied slots exactly, with no exception:
  `F(A)` is ONE argument, while `F(A*)` explicitly spreads `A` into its items
  before binding. A single stored sequence or list value against fixed
  parameters is therefore an arity error (`Add(A)` fails; `Id(A)` binds the
  value whole), and a single-collecting call distinguishes `F(A)`
  (`rest = [A]`) from `F(A*)` (`rest = [a₁, …]`) for a stored sequence and a
  stored list alike (`Coll(((1, 2), 3))` is `[((1, 2), 3)]`). In `KatLang.lean`
  the binders `bindParameterPatternList` / `bindCountedParameterPatternList`
  collect the segment they allocate through `collectSegment`, fed by
  `collectVariadicCallItems` (one item per non-spread argument); in the C#
  runtime the same two binders use `CollectSegment`, fed by
  `BuildCallArgumentInputs`. Callbacks bind through the same counted binder
  (`evalUserCallbackCallCounted`), which is `bindCallback = bindArgs`.
...
* After the receiver-specific supply preparation, call and deconstruction collecting
  ...
  (`bindArgs ps = bindPats ps`,
  `bindDeconstruct = bindPats ∘ openLoneStructure` — definitional in
  `CoreArityAlgebra.lean`). Only deconstruction prepares its supply: it opens
  a lone structure of either kind before any allocation, while a call binds
  its supply as it is.
