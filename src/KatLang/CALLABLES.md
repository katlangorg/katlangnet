...
Item-supply user calls — any callable with a top-level collecting capture, whether single-collecting `G(*x)` or a mixed `F(x, *y, z)` shape — route to one executor path (`IsDeconstructionUserCallShape` / Lean `Algorithm.usesItemSupplyBinding`). That path collects the call argument supply exactly as supplied, then binds prefix/collecting/suffix through the shared `BindParameterPatternList`. A plain sequence-valued or list-valued argument contributes one argument; an explicit `value*` slot contributes the value's immediate items. The collecting binding COLLECTS its assigned slots as ONE list (`CollectSegment` inside `CreateCollectingCapture`; Lean `collectSegment`): zero slots form `[]`, one slot forms `[item]` (never erased), many form `[a, b, ...]`, always with emitted count 1. `G(A)` therefore binds `x = [A]` while `G(A*)` binds `x` to the exact list of A's immediate items — the two calls differ for every sequence-, list-, and empty-valued argument (the pre-list grouped/spread display coincidence is gone; scalar atoms/strings coincide because spread is total), and a call never implicitly opens a single sequence or list argument. An ordinary lexical DOT-CALL receiver is the one exception-shaped-but-general rule: the receiver is injected as ONE leading segment (never pre-expanded; one input for arity and fixed prefix/suffix allocation; a fixed position binds its captured value), and a flat top-level collecting position allocated the segment consumes the segment's evaluated top-level supply (`ParameterPatternInput.CollectingSegmentEmittedCount`, Lean `collectingSegmentCount?` — data-only, never stored, never fed to nested patterns). A WRITTEN group/brace receiver thus supplies its raw rows (`(1, 2, 3).Mean` binds `Vector = [1, 2, 3]`) while a NAMED property receiver supplies its one value-boundary item (`G(A)` and `A.G` agree), and receiver assembly never inspects the callee. Forwarding is ordinary list spread: `Forward(*items) = Target(items*)` re-supplies exactly the collected items (`spread(collect(xs)) = xs`); no raw-supply counts or forwarding provenance exist. Assignment deconstruction (`x, *y, z = RHS`) is a separate unpacking receiver (Python-style): the right-hand side is evaluated once into a shared property, and each target binds through an inline `SequenceValueParameterPattern` that opens that single shared value and matches its items element-by-element. The opening view is `Result.structureItems?` / `StructureItems`: a received sequence value or exact LIST value opens to its immediate items, so `x, y, z = A` unpacks a stored sequence value or list `A`, and `x, y, z = A*` supplies the same items (the written spread form passes through a capture boundary first, which agrees except for a singleton list whose lone element is itself openable, such as `A = [(1, 2)]`, where the spread form opens one level further). The collecting target then collects its middle items through the same `collect` rule (`x, *rest = [1, 2, 3]` binds `rest = [2, 3]`; `x, *rest = 1` binds `rest = []`). This opening is deconstruction-specific and does not leak into calls: it uses the sequence-value parameter pattern, not the item-supply call path, so `F(A)` still passes `A` as one argument. The `(x, *y, z)` sequence-value parameter pattern likewise opens one received value for parameter/callback-position destructuring; only the sequence-value pattern's scalar-element callback fallback stays strict (callback deconstruction for scalar elements deferred) — flat callbacks with a top-level collecting parameter bind through the shared binder (see "Builtins and callbacks" below).
...
`CallableBindingPlan` can describe `Algorithm.User` callback shapes, but callback runtime binding remains executor-owned. Reduce consults the plan only as read-only shape data to detect a top-level variadic accumulator side; the reducer executor still owns accumulator input shaping and binding. Counted callback parameters, sequence-value callback patterns, projection rules, and reducer accumulator behavior stay in the callback executor. Conditional and builtin callbacks intentionally remain outside plan classification because they use orthogonal binding models. A flat callback with a top-level collecting parameter binds through the shared prefix/collecting/suffix binder (`BindCountedCallbackParameterPatternList` / Lean `bindCountedCallbackParameterPatternList`, which delegates to `BindCountedParameterPatternList`): the callback supply first applies the established flat-callback row convention — when fewer argument slots are supplied than top-level parameters, the final supplied argument opens into its items (matching `callee(S:i)`; exact lists stay opaque) — and the collecting parameter then COLLECTS its allocated slots as one list. A single-collecting map/filter callback therefore keeps each iterated element as ONE collected slot (`[7].map(Collect)` binds `items = [7]`; `[(1, 2)].map(Collect)` binds `items = [(1, 2)]`), and a mixed flat callee opens the lone row (`Rows.map(F)` with `F(x, *y, z)` agrees with `F((x, *y, z))`). Reduce supplies element and accumulator callback slots: a genuine single-collecting reducer collects `[element, accumulator]`, while a reducer collecting parameter before a fixed accumulator collects `[element]`. Future callback migration should introduce a `CallbackBindingInput` / policy model first.
