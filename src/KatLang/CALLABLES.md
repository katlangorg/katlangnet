...
Item-supply user calls — any callable with a top-level variadic capture, whether rest-only `G(x...)` or a mixed `F(x, y..., z)` shape — route to one executor path (`IsDeconstructionUserCallShape` / Lean `Algorithm.usesItemSupplyBinding`). That path collects the call argument stream exactly as supplied, then binds prefix/rest/suffix through the shared `BindParameterPatternList`. A plain sequence-valued argument contributes one argument; explicit `...` contributes opened items. Rest-only functions can display the same captured value for `G(A)` and `G(A...)`, but mixed fixed/rest shapes distinguish the supplied boundaries — a function call never implicitly opens a single sequence argument. Assignment deconstruction (`x, y..., z = RHS`) is a separate unpacking receiver (Python-style): the right-hand side is evaluated once into a shared property, and each target binds through an inline `SequenceValueParameterPattern` that opens that single shared value and matches its items element-by-element. So `x, y, z = A` unpacks a stored sequence value `A`, and `x, y, z = A...` supplies the same items. This opening is deconstruction-specific and does not leak into calls: it uses the sequence-value parameter pattern, not the item-supply call path, so `F(A)` still passes `A` as one argument. The `(x, y..., z)` sequence-value parameter pattern likewise opens one received value for parameter/callback-position destructuring; callback-element binding stays on the strict path (callback deconstruction deferred).
...
Builtin metadata uses `CallableSignature` and `CallableBindingPlan` where it is safe to describe builtin call shape. A rest-shaped sequence builtin (`sum(values...)`, `contains(values..., item)`) has builtin-owned collection binding: `BindSequenceBuiltinArguments` / `bindSequenceBuiltinArguments` apply `NormalizeSingletonBoundaryForItemSupply` and then `BindCallableArguments` (the fixed/suffix count is the rest-may-be-empty minimum), so `sum(1, 2, 3)`, `sum((1, 2, 3))`, and `count(Values...)` all consume the same item supply. This singleton-boundary normalization is a sequence-builtin policy, not the user-call parameter binding rule. What stays builtin-owned is the phase *after* binding: collection extraction shape constraints, suffix preparation, callback invocation, and result combination (`CombineCollectionResult` / `combineCollectionResult`, which is shallow singleton-erasing like `combineOutputSlots`: a single kept item — sequence-valued or not — IS the result, while zero or two-plus kept items form one possibly-empty sequence value with sibling boundaries preserved and item internals never renormalized) and result-count rules.
