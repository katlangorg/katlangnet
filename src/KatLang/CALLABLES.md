...
`CallableBindingPlan` can describe `Algorithm.User` callback shapes, but callback runtime binding remains executor-owned. Reduce consults the plan only as read-only shape data to detect a top-level variadic accumulator side; the reducer executor still owns accumulator input shaping and binding. Counted callback parameters, sequence-value callback patterns, projection rules, and reducer accumulator behavior stay in the callback executor. Conditional and builtin callbacks intentionally remain outside plan classification because they use orthogonal binding models. A flat callback with a top-level variadic parameter binds through the shared prefix/collecting/suffix binder (`BindCountedCallbackParameterPatternList` / Lean `bindCountedCallbackParameterPatternList`, which delegates to `BindCountedParameterPatternList`): the callback supply first applies the established flat-callback row convention — when fewer argument slots are supplied than top-level parameters, the final supplied argument opens into its items (matching `callee(S:i)`; exact lists stay opaque) — and the variadic parameter then COLLECTS its allocated slots as one exact immutable list. A single-variadic map/filter callback therefore keeps each iterated element as ONE collected slot (`[7].map(Collect)` binds `items = [7]`; `[(1, 2)].map(Collect)` binds `items = [(1, 2)]`), and a mixed flat callee opens the lone row (`Rows.map(F)` with `F(x, ...y, z)` agrees with `F((x, ...y, z))`). Reduce supplies element and accumulator callback slots: a genuine single-variadic reducer collects `[element, accumulator]`, while a reducer variadic parameter before a fixed accumulator collects `[element]`. Future callback migration should introduce a `CallbackBindingInput` / policy model first.
