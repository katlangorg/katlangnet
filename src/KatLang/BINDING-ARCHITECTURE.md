...
- `FlatCollectingBindingLayout`: plan-derived runtime layout for generic `Algorithm.User` loop-step evaluated-slot binding. User calls use `BindParameterPatternList` and share collecting materialization through `CreateCollectingCapture`.
...
- The binder applies its ordinary rules to that slot: fixed prefix/suffix allocation first (one slot is one input for arity), fixed positions bind the slot's value, a flat top-level collecting position collects its allocated segment EXACTLY — the slot is one collected item whatever its value (VALUES STAY VALUES, September 2026) — and only a written spread slot (`R*`) contributes several slots — expanded by the shared assembler before any binding, like every other spread slot, into items no binder reopens.
- The binder receives NO origin information: a supplied item is its value. The former per-slot `SupplyOrigin` (`WrittenSlot` / `FinalItem`) and its one reader `CollectorSupply` were deleted in September 2026 together with the collector supply-boundary law they served, when VALUES STAY VALUES superseded it: once no binder reinterprets one item as several, nothing needs to know how an item entered the supply. Nothing origin-shaped rides on `ParameterPatternInput` (whose `Source` is diagnostic identity for the F5 written-argument blame only) or enters a `CountedResult` or any result value.
...
A future model must not carry receiver information at all: the receiver is indistinguishable from a written argument once assembled. It must not reintroduce slot provenance either (`SupplyOrigin` was deleted with the collector supply-boundary law): a supplied item is its value, and only the explicit spread and explicit structural patterns open one.
...
The remaining patterned behavior is executor-owned runtime policy: explicit argument evaluation timing, explicit block-to-sequence-value item extraction, top-level algorithm-channel binding, nested algorithm suppression during sequence-value recursion, loop value-only state-slot semantics, callback item value-boundary recounting, the sequence-value scalar one-item fallback (ONE rule, `SequenceValuePatternItems` / Lean `Result.sequenceValuePatternItems`, shared by the ordinary and the counted callback binders since S3, September 2026), Lean's binding failure order with order-independent repeated names (D1, September 2026: every pattern of a range binds before its repeated names are decided, each repeated name is decided once — when its last contribution joins — by requiring every pair of its contributions to be compatible in every channel (values, complete counted pairs and callable identity), preserving the complete resulting binding under contribution permutations, and a collecting list decides the names shared across the collector after the collector is materialized), and arity or wrong-shape diagnostic selection.
...
Callback INVOCATION remains executor-owned runtime policy across counted callback evaluation, conditional callback dispatch, map callbacks, filter callbacks, reduce step callbacks, and builtin-as-callback paths: the executor decides how many values each invocation supplies (one for `map`/`filter`, two for `reduce`) and how often it invokes. Callback BINDING is not a policy of its own (THE CALLBACK LAW, September 2026): every user callee binds the supplied values through the ordinary counted binder. `CallableBindingPlan` may describe callback signature shape, but it does not own callback input shaping or execution policy.

Collecting parameter kinds are never discarded on any callback route (July 2026 correction), and since September 2026 (THE CALLBACK LAW) every user callee binds each callback element through the ONE ordinary counted binder `BindCountedParameterPatternList` (Lean `bindCountedParameterPatternList`, called by `evalUserCallbackCallCounted`; C# `EvalResolvedCallbackCallCountedCore` and its async twin; a single-clause family through `TryGetFlatBinderUserEquivalent` / Lean `flatBinderUserEquivalent?`): the element is ONE ordinary argument and the reducer's accumulator another, so a collecting parameter COLLECTS exactly the supplied values as one list, exactly as the direct call does. The former flat-callback row expansion (`BindCountedCallbackParameterPatternList` / `bindCountedCallbackParameterPatternList`), the fixed-only `BindCountedCallbackParams` route, and the reducer's accumulator-slot inputs are deleted — there is no callback-specific binding algorithm.
...
Callbacks receive already-evaluated `CountedResult` values; callback items preserve their stored value and carry its ordinary value-boundary count (zero for `()`, one otherwise); the reducer accumulator is ONE ordinary argument `(acc, valueCount acc)` beside the element, never shaped differently; `EmittedCount` threads through counted callback paths; callbacks do not allow algorithm-channel binding; and callback diagnostics are selected and wrapped by the relevant executor call site. Counted and uncounted binders are not unified now because `CountedResult` versus `Result` is a structural difference, not accidental duplication — but they must never disagree about whether a value binds a pattern: the nested-pattern OPENING is the one shared `SequenceValuePatternItems` rule, and slot allocation is the one shared `ParameterPattern.MinimumSuppliedSlots` / `CollectSegment` machinery — since September 2026 the callback binds through that very binder — so a callback that supplies one value binds it exactly as the ordinary call supplying that value does (S3, September 2026: the counted binder's former singleton-only scalar fallback made `map([7], P)` with `P((x, *rest))` a bare `BadArity` while `P(7)` bound `x = 7, rest = []`). `CallbackNestedPatternBindingTests` is the direct-versus-callback metamorphic suite.
