...
COMMA MATTERS AFTER A POSTFIX RUN: a bare identifier on the same line directly
after a postfix grace run is the omitted-dot member of a graced dot edge
(`b~ a` means `b~.a`, the graced dot edge — see GRACE WITH DOTCALL), so a
graced slot followed by a separate expression needs the explicit comma:
`b~, a + b`. This mirrors the spread marker's `A*, B` rule.
...
- GRACE WITH DOTCALL: `a~.f(args)` is ordinary postfix Grace on the bare receiver name followed by an ORDINARY DOT EDGE; `a.~f(args)` is ordinary dot with prefix Grace on the participating member/fallback occurrence; `a~f(args)` is the OMITTED-DOT spelling of the same graced edge (the marker between the two names carries the edge, so `a~f` parses like `a~.f` and elaborates like `a.~f` too). The body stays the STRUCTURAL-FIRST dot edge in every spelling: `K = a~f` infers `(f, a)`, and the direct call `f(a)` describes only its lexical-FALLBACK arm — never a universal equivalence to `K(f, a) = f(a)` (a receiver that owns `f` structurally resolves to its own member where the direct call would invoke the lexical callable). The shorthand is grammar-local, not a textual rewrite of `~` to `~.` (in an indexing selector, `x:a~f` selects with the whole edge while `x:a~.f` dot-chains the index result). Base semantic occurrence order is receiver, participating fallback, then written arguments: `K = a.f` infers `(a, f)`, while the graced two-name forms infer `(f, a)` through the ONE general Grace pass. Runtime fallback still invokes `f(a, args)`, but that assembly does not order the enclosing signature. Postfix Grace requires its receiver operand to be one bare name, so `(x + y)~.t`, `f(x)~.t`, `[x, y]~.t`, `5~.t`, and a second `~.` in `a~.t~.u` reject; prefix member Grace remains valid with a compound receiver because it decorates the bare member name. Call arguments are unrestricted (`a~.t(b, c)`, `a.~t(b, c)`, and `a~t(b, c)` infer `(t, a, b, c)`). Grace NEVER changes member selection or any other executable semantics: `Obj~.V`, `Obj.~V`, and `Obj~V` read Obj's structural `V` exactly like `Obj.V` (write `V(Obj)` to reach a lexical `V`), `x~.string` / `x.~string` / `x~string` keep the dot-only intrinsic, `S~.count` / `S.~count` / `S~count` keep the dotted builtin view, and receiver-segment supply is unchanged. A member participates in inference when fallback MAY be selected, but not when structural resolution is certain and not in a CLOSED explicit list. `a ~ .t` and `a ~ t` follow the ordinary whitespace-insensitive postfix-Grace grammar and therefore have the same inferred order as `a~.t`; prefix member Grace must begin directly after the dot. The omitted-dot member must sit on the same physical line as the run, and an identifier that begins a declaration keeps its declaration meaning (`a~ K = 5` is a graced row plus the definition `K = 5`, never `a~.K = 5`). A grace-marked `open` target is rejected (`open M~.C`, `open M~C`).
