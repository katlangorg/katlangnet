...
- Math arities are correct in every spelling, especially `log(value, base)`, `pow(x, y)`, `atan2(y, x)`, `round(x, digits)`, `random(start, end)`, and `randomInt(start, end)` (identically for `Math.Log`, `Math.Pow`, `Math.Atan2`, `Math.Round`, `Math.Random`, and `Math.RandomInt`).
...
- Every Math member has one predefined lower-camel-case prelude binding pointing to the SAME canonical function, not a copy: `pi`, `e`, `abs`, `ceil`, `floor`, `round`, `sign`, `sqrt`, `ln`, `lg`, `sin`, `asin`, `cos`, `acos`, `tan`, `atan`, `atan2`, `pow`, `log`, `random`, `randomInt` for `Math.Pi`, `Math.E`, `Math.Abs`, ..., `Math.RandomInt`.
...
- `round` always takes two arguments: `round(x, digits)` / `Math.Round(x, digits)`, where `digits` is the integer number of digits to keep after the decimal point and must be `>= 0`. Use `0` for integer rounding. Midpoints round away from zero, so `round(1.225, 2)` is `1.23`.
...
- Multi-argument Math members — always supply every argument:
    ...
    - `pow(x, y)` / `Math.Pow(x, y)` raises `x` to the power `y` and is identical to `^` (they share one implementation): an integer exponent of magnitude up to ~9.2e18 uses exact arithmetic (`2 ^ 10` is exactly `1024`), while a fractional or astronomically large integer exponent is a 34-digit approximation. Prefer `^` for ordinary powers; use `pow`/`Math.Pow` when a Math-member style is specifically wanted.
