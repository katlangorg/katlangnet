...
- Prefix `...name` is the ONLY rest-binding syntax: generate `F(...items) = body`, `F(first, ...middle, last) = body`, and `first, ...middle, last = value`. Postfix rest such as `F(items...) = body` or `first, middle..., last = value` is a PARSE ERROR, never a variadic definition. Keep value spreading postfix, for example `Forward(...items) = Target(items...)`.
