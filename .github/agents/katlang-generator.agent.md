...
- For ordinary user-defined dot-call fallback, DOT-CALL PASSES A VALUE: `A.B(C, D)` is exactly `B(A, C, D)` — the receiver is one ordinary leading argument, it never satisfies extra fixed-parameter arity, a fixed parameter binds it as one value, and a collecting parameter binds it exactly as it binds the written argument (the collector supply-boundary rule below: a lone sequence-valued receiver opens one level, a list stays one item). Do not generate `(a, b).F` expecting fixed parameters `F(a, b)`; use `F(a, b)` or `a.F(b)`. With `Mean(*Vector) = Vector.sum / Vector.count`, `Mean(1, 2, 3)`, `(1, 2, 3)*.Mean`, `Arg*.Mean`, `(1, 2, 3).Mean`, and `Arg.Mean` (for `Arg = 1, 2, 3`) all average the three items; `[1, 2, 3].Mean` passes ONE list (`Vector = [[1, 2, 3]]`) and the numeric sum fails — spread a list (`[1, 2, 3]*.Mean`) when the helper needs its items.
...
- For a reusable collection helper that collects supplied arguments, declare a lone collecting parameter, such as `Many(*values) = values.count`. A user `*values` collecting parameter consumes its allocated argument supply by the COLLECTOR SUPPLY-BOUNDARY RULE: multiple supplied items are collected exactly; if its entire segment is one lone non-spread sequence value, that sequence provides the collector's whole supply, opening exactly one level; lists remain exact; items already produced by explicit spread are final supplied items. With `Arg = 1, 2, 3`, `Many(Arg*)`, `Many(1, 2, 3)`, `Many(Arg)`, and `Arg.Many` all bind `values = [1, 2, 3]` (count 3); `Many(Arg, 0)` binds `[(1, 2, 3), 0]` (count 2); `Many([1, 2, 3])` binds `[[1, 2, 3]]` (count 1) while `Many([1, 2, 3]*)` binds the items; `Many(())` is `[]` and `Many((), 3)` is `[(), 3]`; the opening is never recursive (`Many(((1, 2), 3))` binds `[(1, 2), 3]`). A parameter list with two or more captures containing one collecting parameter binds the fixed positions from the front and back first — a fixed position never opens a sequence argument — and then applies the same rule to the remaining middle segment (`Tail(first, *rest) = rest` gives `Tail(1, (2, 3))` → `[2, 3]` and `Tail(1, (2, 3), 4)` → `[(2, 3), 4]`); use an explicit spread star when a sequence value should fill FIXED slots. The same comma binding pattern on the left of `=` is, by contrast, an unpacking receiver (Python-style): assignment deconstruction opens one lone sequence- or list-valued right-hand side before binding, so with `A = 1, 2, 3`, `x, y, z = A` binds `x = 1`, `y = 2`, `z = 3` and `x, y, z = A*` supplies the same items (`first, *rest = A` unpacks to `first = 1`, `rest = [2, 3]` — the collecting binding collects an exact list). This unpacking is assignment-specific — an ordinary call `F(A)` still passes `A` as one argument, so calls need `F(A*)`. At most one collecting binding is allowed. Do not use `atoms` unless recursive flattening is intentionally required.
...
- For `filter`, `map`, and `reduce`, keep that same top-level iteration structure, and bind each callback item as the one selected value that `S:i` would produce (a nested sequence or list item stays one intact value; `Id(x) = x` maps `((1, 2), 3)` to `[(1, 2), 3]`). `filter` still keeps or discards the original top-level item, `reduce` leaves accumulator semantics unchanged, and nothing recursively flattens. Dot-call sequence builtins on the callback variable bind the item as their one collection argument and open it one level, so `item.count` counts a sequence-valued item's members. If you need members of a sequence-value callback item, use ordinary parameters (a flat multi-parameter callback opens a sequence-valued item into row slots), a nested pattern `F((x, y))`, or `item:i`.
- Callbacks with a collecting parameter collect exact lists like ordinary calls: each iterated element is ONE written argument of the callback call, so the collector supply-boundary rule applies. A single-collecting-parameter map/filter callback (`Collect(*items) = items`) keeps a scalar or list element as one collected slot — `[7].map(Collect)` is `[[7]]`, `[[1, 2]].map(Collect)` is `[[[1, 2]]]` — and opens a sequence element one level — `[(1, 2)].map(Collect)` is `[[1, 2]]` — so predicates can compare kinds exactly (`IsSingleSeven(*items) = items == [7]`; `IsPair(*items) = items.count == 2` keeps the pairs of `((1, 2), 3, (4, 5), [6, 7])`). A multi-parameter flat callback (`F(first, *middle, last)`) opens a lone sequence-valued element into row slots first (the same row rule as fixed callbacks) and then collects the middle: `[(1, 2, 3, 4)].map(F)` binds `middle = [2, 3]`, agreeing with the nested `F((first, *middle, last))` form. Reduce supplies two callback slots, element and accumulator: `R(*items) = items` therefore binds `items = [element, accumulator]`, while `R(*items, acc)` receives one written element slot: a sequence opens one level (`(1, 2)` binds `items = [1, 2]`), and a scalar or list stays `[element]`.
...
Core rules:
- Prefix `*` collects ONLY in binding patterns — explicit parameter lists, nested sequence-value patterns, and assignment deconstruction — and the star must be directly attached to its name. A collecting binding COLLECTS the argument slots assigned to it into one list: zero slots form `[]`, one slot forms `[item]` (never collapsed to the item), many form `[a, b, ...]`. `Name(*values) = body` binds `values` to that collected list, read through the collector supply-boundary rule: `Name(Arg)` supplies ONE written argument — for a stored SEQUENCE `Arg` the lone written sequence is the collector's whole supply and opens one level (`values = [1, 2, 3]` for `Arg = 1, 2, 3`), for a stored LIST it stays one item (`values = [Arg]`) — while `Name(Arg*)` explicitly spreads `Arg` first (`values` becomes the exact list of Arg's immediate items, final items that never open further: `Name([(1, 2)]*)` is `[(1, 2)]`). Grouped and spread calls therefore coincide for scalars and sequence values and differ for every list-valued argument; beside another argument a sequence value is collected exactly (`Name(Arg, 0)` is `[(1, 2, 3), 0]`). An empty call `Name()` collects `[]`, `Name(())` collects `[]` too (the lone written `()` opens to nothing), and a one-item supply stays wrapped: with `Collect(*items) = items`, `Collect(1)` is `[1]` and `Collect((), 3)` is `[(), 3]`.
...
- Normal parameters before `*values` bind from the front; normal parameters after it bind from the back, and the collecting parameter then consumes the remaining middle segment by the collector supply-boundary rule (several slots exactly; a lone non-spread sequence value opened one level; lists exact). With `Scale(*values, factor) = values.map{n * factor}` and `Arg = 1, 2, 3`, `Scale(Arg*, 10)`, `Arg*.Scale(10)` (the spread receiver's items become the leading call arguments), `Scale(1, 2, 3, 10)`, `Scale(Arg, 10)`, and `Arg.Scale(10)` all bind `values = [1, 2, 3]`; `Scale([1, 2, 3], 10)` binds `values = [[1, 2, 3]]` (the numeric callback then fails on the list element — spread a list when the helper needs its items), and `Scale(Arg, 4, 10)` binds `values = [(1, 2, 3), 4]`. Collection builtins are separate ordinary fixed-arity callables with one `collection` argument plus fixed controls.
- Nested sequence values remain intact; sibling grouped values are not auto-flattened. With `Arg = (1, 2), (3, 4)` and `Many(*values) = values.count`, `Many(Arg*)` is `2` — the spread opens `Arg` into the item supply, keeping its two grouped values as siblings — and `Many(Arg)` is `2` as well (the lone sequence argument opens exactly one level, to the same two pairs), while `Many(Arg, 0)` is `2` with `values = [((1, 2), (3, 4)), 0]`.
...
- A variadic-with-suffix user callable binds an item supply. With `Sum(*values, last)` and `Values = 10, 20`, `Sum(Values*)` binds `values = [10]` and `last = 20`, while `Sum(Values*, 7)` binds `values = [10, 20]` and `last = 7` (the spread items join the supply); `Sum(Values, 7)` binds the same `values = [10, 20]` because `last` takes 7 first and the lone written sequence left to the collector opens one level, while `Sum(Values)` gives `last` the whole pair.
...
- Higher-order callbacks bind the current item as one selected value (the `S:i` boundary), and dot-call sequence builtins on that callback variable bind the item as their one collection argument and open it one level. A collecting-parameter callback collects through the shared binder, the whole element being ONE written argument of the callback call: single-collecting-parameter map/filter callees keep a scalar or list element as one collected slot (`[7].map(Collect)` binds `items = [7]`) and open a sequence element one level (`[(1, 2)].map(Collect)` binds `items = [1, 2]`), mixed flat callees open a lone sequence element into row slots (final items) before front/collecting/back allocation, and a lone-collecting-parameter reducer collects both supplied slots as `[element, accumulator]`.
