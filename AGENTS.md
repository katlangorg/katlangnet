...
- Ellipsis `...` is the postfix spread operator. `expr...` opens ONE item-producing boundary of the evaluated value and contributes the opened items to the surrounding item supply — it does not create or emit a sequence value by itself. Sequence values and exact list values supply their contained items; other values follow the total item-view rule (an atom or string supplies itself as one item). It never consumes a right operand: `A...B`, `A...C`, `A... B`, and `A...` newline `B` all parse as expression lists beginning with `A...`. `A... ; B` is invalid semicolon syntax.
- Square brackets construct EXACT immutable list values (`Result.listValue` / `Expr.ListLiteral`), a second collection kind beside sequence values: no singleton/empty canonicalization ever applies to list structure (`[7] != 7`, `[] != ()`), while parentheses around a list stay redundant sequence grouping (`([1, 2]) == [1, 2]`). `[` always begins a NEW expression (never a call/indexing delimiter; `A[1]` is adjacency `A, [1]`), and bracket content is a pure expression list (spread elements insert their item supply; declarations are illegal inside brackets). Spread uses `Result.spreadItems` / `SpreadItems` (opens one sequence OR list boundary); every NON-spread consumer keeps lists opaque via `toItems` (a list is one item for indexing projection, value boundaries, and call binding — calls never open lists implicitly). Deconstruction's `Result.structureItems?` / `StructureItems` opens a lone sequence OR lone list (`x, y, z = [1, 2, 3]`); rest captures stay sequence-shaped. Builtin collection binding does NOT open lists: a list in a bound item supply is a targeted deferred-support type error until the follow-up builtin work (explicit spread like `count([1, 2, 3]...)` is fully supported; `atoms` flattens and omits lists like strings). Receiver theorems: sequence-side `receivers_agree_of_not_lone_seq` / `agree_on_lone_seq_iff_lone_rest` in `lean/CoreArityAlgebraProofs.lean`; list-side receiver behavior is proven by the list bridge laws in `lean/KatLangArityLaws.lean` (e.g. `deconstruct_fixed_single_list_opens`, `lone_rest_list_call_and_deconstruct_differ`). Exact list values are intentionally NOT modeled in the CoreArityAlgebra paper artifact.
...
- A user-defined top-level variadic parameter captures the argument stream that was actually supplied to the call. `Name(values...) = body` binds supplied arguments as one grouped sequence value; a plain `Name(A)` supplies one argument, while `Name(A...)` explicitly opens `A` first. Rest-only functions can display the same value for both paths, but mixed fixed/rest shapes distinguish them. `Name()` binds the empty stream. A lone rest-only `values...` is the degenerate single-rest case of this model.
- A parameter list with two or more captures that contains one rest binds the supplied function-call argument stream: fixed captures bind from the front and back, and the single movable rest captures the remaining middle arguments as one grouped sequence value (possibly zero); a plain function call does not implicitly open a single sequence argument. Assignment deconstruction, by contrast, is an unpacking receiver (Python-style): `x, y..., z = RHS` opens one lone sequence- or list-valued right-hand side and matches its items element-by-element, so `x, y, z = A` splits a stored sequence value or exact list value `A` and `x, y, z = A...` supplies the same items. This unpacking is deconstruction-specific and does NOT change function calls — `F(A)` still passes `A` as one argument and needs `F(A...)` to open it. The shared front/rest/back matcher is `bindParameterPatternList` (Lean) / `BindParameterPatternList` (C#); item-supply call routing is `Algorithm.usesItemSupplyBinding` / `IsDeconstructionUserCallShape`. Assignment deconstruction is parser-elaborated so the right-hand side is evaluated once into a shared property and each target binds through an inline sequence-value parameter pattern (`SequenceValueParameterPattern`) that opens that single shared value. The same `(x, y..., z)` sequence-value parameter pattern opens one received value for parameter/callback-position destructuring; callback-element binding is intentionally left on the strict path (callback deconstruction deferred).
- Ordinary lexical dot-call passes the receiver as one leading argument boundary. `A.B(C, D)` means `B(A, C, D)`, not a call where `A`'s top-level values are spread before `C` and `D`. If `B` has a leading `values...` parameter, that parameter binds the receiver slot as part of its item supply after ordinary receiver injection.
- Sequence builtins such as `filter`, `map`, `order`, `orderDesc`, `count`, `first`, `last`, `min`, `max`, `sum`, `avg`, and `reduce` expose a rest-shaped public signature (`sum(values...)`, `contains(values..., item)`) and consume a builtin-owned **item supply**. The collection binding applies singleton-boundary normalization for builtin collection inputs — `count(Values)`, `count((1, 2, 3))`, and `count(1, 2, 3)` all count three items, and `count(Values...)` opens its items into the same supply — while user-defined function calls preserve argument boundaries unless `...` is written. Suffix parameters bind from the back (`contains(values..., item)` binds `contains(1, 2, 3, 2)` and `contains((1, 2, 3), 2)` alike), and multiple sibling grouped values are preserved unless explicitly opened with `...` (so `sum(A, B)` with grouped `A`/`B` is rejected by the numeric-item constraint rather than silently flattened, while `sum(A..., B...)` concatenates). A builtin that genuinely requires one collection value must declare a non-rest `values` parameter instead of `values...`. Collection-producing builtins combine their returned items with the shallow singleton-erasing `combineCollectionResult` / `CombineCollectionResult` path (same shape as `combineOutputSlots`): zero items form `()`, a single kept item IS the result (so `take(((1, 2), (3, 4)), 1)` is `(1, 2)` and `distinct((), ())` is `()` — builtin results never mint literal-unwritable orphans like `((1, 2))` or `(())`), and two or more items form one sequence value whose sibling boundaries — including `()` items — are preserved raw with item internals never renormalized. Callback/builtin execution stays a separate runtime-owned phase after binding.
