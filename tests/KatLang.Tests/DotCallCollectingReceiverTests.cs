...
/// argument, while a collecting parameter collects it exactly as it collects the
/// written argument (<see cref="ExplicitValueOpeningTests"/>): ONE item whatever its
/// value (<c>(1, 2).Coll</c> is <c>[(1, 2)]</c>, <c>().Coll</c> is <c>[()]</c>,
/// <c>[].Coll</c> is <c>[[]]</c>), alone or beside another written argument
/// (<c>(1, 2).Coll(3)</c> is <c>[(1, 2), 3]</c>). Only the spread marker opens a
/// receiver into items: the fluent <c>recv*.F(extra...)</c> is exactly
/// <c>F(recv*, extra...)</c>. The receiver's
...
public class DotCallCollectingReceiverTests
{
    ...
    [Fact]
    public void EmptySequenceReceiver_IsOneVisibleItem_AndOnlyItsSpreadSuppliesNothing()
    {
        ...
        // The collector collects it as one visible item, alone or beside another
        // argument; only its spread supplies zero items.
    ...
    [Fact]
    public void SequenceReceiver_IsOneItem_AloneOrBesideAnother()
    {
        ...
        // A spread-produced sequence value is never reopened, even when it is the lone item.
        AssertResult(CollectDef + "Collect([(1, 2)]*)", List(Seq(Atom(1), Atom(2))));
        AssertResult(CollectDef + "[(1, 2)]*.Collect", List(Seq(Atom(1), Atom(2))));
    }
    ...
    public void GroupReceivers_AreOneValue_NeverTheirRows()
    {
        ...
        // written argument `Collect((1, 2))`, collected as one item. Nesting adds
        // nothing: `((1, 2))` is the same one value; `(1, (2, 3))` keeps its nested
        // pair.
        AssertResult(CollectDef + "((1, 2)).Collect", List(Seq(Atom(1), Atom(2))));
        ...
        // `(Values*)` captures the spread supply as ONE value, one collected item;
        // the fluent `Values*.Collect` — the call `Collect(Values*)` — supplies the
        // items.
    ...
    public void BraceBlockReceiver_IsOneValue()
    {
        ...
        // argument `Collect({1, 2, 3})` — collected as one item.
    ...
    public void NamedPropertyReceiver_IsOneValue()
    {
        ...
        // (`One(a) = 1` binds it) — collected as one visible item.
    ...
    [Fact]
    public void CollectingWithSuffix_ReceiverIsOneItemAfterSuffixAllocation()
    {
        // Two arguments: the receiver and 10. The suffix binds 10 from the
        // back FIRST; the collector collects the receiver's one value exactly —
        // exactly as `Scale((1, 2, 3), 10)` does — a sequence and a list receiver
        // alike, and beside a second written argument too.
        AssertResult(
            """
    ...
    public void PatternedBraceBlockReceiver_IsOneValue()
    {
        ...
        // collects it exactly: the block sequence and a list are one item each,
        // alone or beside a second middle slot.
        AssertResult(
            """
    ...
    [Fact]
    public void SingleCollectingMapCallback_CollectsEachElementAsOneItem()
    {
        // A whole callback item is ONE argument of the callback call: a scalar, a
        // sequence, and a list element are each one collected item.
