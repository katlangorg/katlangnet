...
public class FrontEndTraversalTests
{
    ...
    private static IReadOnlyDictionary<string, Func<Expr, Expr>> RecursiveEmbeddings { get; } =
        new Dictionary<string, Func<Expr, Expr>>(StringComparer.Ordinal)
        {
            ...
            ["Comparison.First"] = child => new Expr.Comparison(child, [new ComparisonLink(ComparisonOp.Lt, new Expr.Num(1))]),
            // A LATER link, so the walk is proven to reach every link rather than only the first.
            ["Comparison.Link"] = child => new Expr.Comparison(
                new Expr.Num(1),
                [new ComparisonLink(ComparisonOp.Lt, new Expr.Num(2)), new ComparisonLink(ComparisonOp.Le, child)]),
    ...
    public static TheoryData<string, string> LiftingPositions() => new()
    {
        ...
        { "Comparison.First", "F(x) = x + 1\nF < 2" },
        { "Comparison.Link", "F(x) = x + 1\n1 < 2 < F" },
