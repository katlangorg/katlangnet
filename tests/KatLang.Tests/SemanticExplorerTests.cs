...
public class SemanticExplorerTests
{
    ...
    private static void CheckValueFamily(string valueId, List<Finding> findings)
    {
        ...
        // Collecting binding COLLECTS exactly (THE EXACT COLLECTOR LAW): a
        // single-collecting callee receives its ONE grouped argument as one item,
        // so every value — a sequence, `()`, and an exact list alike — is the
        // one-element exact list holding it (`F(*x) = x` with `F(V)` observes
        // `[V]`), never the value itself and never its items.
    ...
    public static TheoryData<string, string> AnchorCases => new()
    {
        ...
        // DOT-CALL PASSES A VALUE and THE EXACT COLLECTOR LAW collects it: the
        // selected `()` is ONE argument in the dotted spelling exactly as in the
        // written call, collected as one visible item alone or beside another
        // argument; only its spread supplies nothing, and a selected list is the
        // same rule.
