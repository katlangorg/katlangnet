...
/// <summary>
/// Deconstruction binding patterns: a comma binding pattern with one movable collecting binding
...
/// <c>x, y, z = A.spread</c> supplies the same items. This unpacking is
/// deconstruction-specific: a function call <c>F(A)</c> still passes <c>A</c> as one
/// argument and needs <c>F(A.spread)</c> to spread it. The collecting binding collects its items
...
public class DeconstructionBindingTests
{
    ...
    public void Assignment_MovableCollectingBinding_ExplicitSpread_SuppliesSameItems()
        // Explicit `.spread` supplies the same items as the bare unpack above.
    ...
    {
        // `first, rest... = A.spread` supplies the same items as the bare unpack above.
        const string define = "A = 1, 2, 3\nfirst, rest... = A.spread\n";
        AssertAtoms(define + "first", 1);
        AssertAtoms(define + "rest", 2, 3);
    }
    ...
    {
        // `head..., last = spread(A)` supplies the same items as the bare unpack above.
        const string define = "A = 1, 2, 3\nhead..., last = spread(A)\n";
        AssertAtoms(define + "head", 1, 2);
        AssertAtoms(define + "head.count", 2);
        AssertAtoms(define + "last", 3);
    ...
    public void Assignment_DeconstructionAfterOutputLine_DoesNotAbsorbIntoOutput()
    {
        ...
        // the output stays the single `F(A.spread)` row (15), and `x, y..., z = A` defines
    ...
    [Fact]
    public void Parameter_VariadicCollectsEmptyList()
    ...
    [Fact]
    public void Parameter_ScalarArgument_VariadicCollectsEmptyList()
    ...
    public void Parameter_MixedVariadicShape_DistinguishesPlainArgumentFromSpread()
    {
        ...
        AssertAtoms(g + "G(A.spread)", 1, 1);   // first = 1, rest = [2]
    }
    ...
    public void SingleVariadic_GroupedArgumentDiffersFromItemSupply()
    {
        ...
        // collected list holds one non-numeric element and .sum fails. A.spread and
    ...
    public void RepeatedSingletonBoundary_DoesNotImplicitlyOpenCallArgument()
    {
        ...
        AssertEvalError<EvalError.BadArity>("G(x...) = x.sum\nG(((1, 2, 3, 4, 5)))");
        AssertAtoms("G(x...) = x.sum\nG(((1, 2, 3, 4, 5)).spread)", 15);
        AssertEvalError<EvalError.TypeMismatch>("F(x..., y) = x.sum + y\nF(((1, 2, 3, 4, 5)))");
        AssertAtoms("F(x..., y) = x.sum + y\nF(((1, 2, 3, 4, 5)).spread)", 15);
