...
public class ParserTests
{
    ...
    [InlineData("A ~B")]
    [InlineData("A ~ B")]
    public void Parse_PostfixGraceRunThenSameLineIdentifier_IsTheOmittedDotGracedEdge(string source)
    {
        // Same-line '~' after an identifier is postfix grace on that
        // identifier, and a same-line identifier after the run is the
        // omitted-dot MEMBER of the same graced dot edge: `A~B` ≡ `A~.B`.
        // Whitespace between these postfix tokens is insignificant, exactly
        // as in the dotted `A ~ .B` spelling.
        var result = Parser.ParseSyntax(source);

        Assert.False(result.HasErrors);
        var edge = Assert.IsType<Expr.DotCall>(Assert.Single(result.Root.Output));
        Assert.Equal("B", edge.Name);
        Assert.Null(edge.Args);
        var grace = Assert.IsType<Expr.Grace>(edge.Target);
        Assert.Equal("A", Assert.IsType<Expr.Resolve>(grace.Inner).Name);
        Assert.Equal(1, grace.Weight);
        Assert.Equal("B", Assert.IsType<Expr.Resolve>(edge.LexicalFallback).Name);
    }

    [Fact]
    public void Parse_OmittedDotGracedEdge_TakesArgumentListsLikeTheDottedSpelling()
    {
        var withArgs = Parser.ParseSyntax("A~B(1, 2)");
        Assert.False(withArgs.HasErrors);
        var edge = Assert.IsType<Expr.DotCall>(Assert.Single(withArgs.Root.Output));
        Assert.Equal("B", edge.Name);
        Assert.NotNull(edge.Args);
        Assert.Equal(2, edge.Args.Count);

        // The property-style vs explicit-empty-argument-list distinction of
        // the ordinary dot edge carries over unchanged.
        var explicitEmptyParse = Parser.ParseSyntax("A~B()");
        Assert.False(explicitEmptyParse.HasErrors);
        var explicitEmpty = Assert.IsType<Expr.DotCall>(
            Assert.Single(explicitEmptyParse.Root.Output));
        Assert.NotNull(explicitEmpty.Args);
        Assert.Empty(explicitEmpty.Args);
    }

    [Fact]
    public void Parse_OmittedDotGracedEdge_RepeatedAndMixedRunsUseOrdinaryWeights()
    {
        // `A~~B` is a weight +2 run on the receiver, like `A~~.B`.
        var repeatedParse = Parser.ParseSyntax("A~~B");
        Assert.False(repeatedParse.HasErrors);
        var repeated = Assert.IsType<Expr.DotCall>(Assert.Single(repeatedParse.Root.Output));
        Assert.Equal(2, Assert.IsType<Expr.Grace>(repeated.Target).Weight);

        // `~A~B` nets weight 0 — the run still carries the omitted dot, so it
        // is the plain edge `A.B`, exactly like `~A~.B`.
        var netZeroParse = Parser.ParseSyntax("~A~B");
        Assert.False(netZeroParse.HasErrors);
        var netZero = Assert.IsType<Expr.DotCall>(Assert.Single(netZeroParse.Root.Output));
        Assert.Equal("A", Assert.IsType<Expr.Resolve>(netZero.Target).Name);
        Assert.Equal("B", netZero.Name);
    }

    [Fact]
    public void Parse_PostfixGraceAtLineEnd_DoesNotContinueToTheNextLineIdentifier()
    {
        // A physical newline never continues a closed expression: the graced
        // occurrence stays its own output row and `B` is a separate row.
        var result = Parser.ParseSyntax("A~\nB");
    ...
    [InlineData("A~, B")]
    [InlineData("A ~, B")]
    public void Parse_PostfixGraceThenComma_KeepsTwoExpressionSlots(string source)
    {
        // The explicit comma stays the way to write a graced slot followed by
        // an ordinary slot, mirroring the spread marker's `A*, B` rule.
        var result = Parser.ParseSyntax(source);

        Assert.False(result.HasErrors);
        Assert.Equal(2, result.Root.Output.Count);
        Assert.IsType<Expr.Grace>(result.Root.Output[0]);
        Assert.Equal("B", Assert.IsType<Expr.Resolve>(result.Root.Output[1]).Name);
    }

    [Fact]
    public void Parse_PostfixGraceThenNonIdentifier_KeepsAdjacencySlots()
    {
        // Only an identifier can be the omitted-dot member; any other
        // same-line token keeps the ordinary adjacency expression list.
        var result = Parser.ParseSyntax("A~ 5");

        Assert.False(result.HasErrors);
        Assert.Equal(2, result.Root.Output.Count);
        Assert.IsType<Expr.Grace>(result.Root.Output[0]);
        Assert.Equal(5, Assert.IsType<Expr.Num>(result.Root.Output[1]).Value);
    }

    [Theory]
    [InlineData("A~ K = 5")]
    [InlineData("A~K = 5")]
    public void Parse_PostfixGraceThenPropertyDefinition_KeepsTheDefinition(string source)
    {
        // An identifier that begins a declaration is never an omitted-dot
        // member — the algorithm loop owns definition forms, exactly as in
        // the implicit expression-list separator rule.
        var result = Parser.ParseSyntax(source);

        Assert.False(result.HasErrors);
        var grace = Assert.IsType<Expr.Grace>(Assert.Single(result.Root.Output));
        Assert.Equal("A", Assert.IsType<Expr.Resolve>(grace.Inner).Name);
        var property = Assert.Single(result.Root.Properties);
        Assert.Equal("K", property.Name);
    }

    [Fact]
    public void Parse_PostfixGraceThenClauseDefinition_KeepsTheClause()
    {
        var result = Parser.ParseSyntax("A~ F(x) = x + 1");

        Assert.False(result.HasErrors);
        Assert.IsType<Expr.Grace>(Assert.Single(result.Root.Output));
        var property = Assert.Single(result.Root.Properties);
        Assert.Equal("F", property.Name);
    }

    [Fact]
    public void Parse_PostfixGraceThenDeconstructionAssignment_KeepsTheBindings()
    {
        // An identifier that begins a comma deconstruction assignment is a
        // declaration form the algorithm loop owns, never an omitted-dot
        // member — the LookaheadIsBindingPatternAssignment exclusion.
        var result = Parser.ParseSyntax("A~ x, *y = 1, 2, 3");

        Assert.False(result.HasErrors);
        var grace = Assert.IsType<Expr.Grace>(Assert.Single(result.Root.Output));
        Assert.Equal("A", Assert.IsType<Expr.Resolve>(grace.Inner).Name);
        Assert.Contains(result.Root.Properties, p => p.Name == "x");
        Assert.Contains(result.Root.Properties, p => p.Name == "y");
    }

    [Fact]
    public void Parse_UngracedAdjacentIdentifiers_StayTwoSlots()
    {
        // The omitted-dot member exists only after a NON-EMPTY postfix grace
        // run (mirrored by the EBNF's PostfixGraceRun factoring): plain
        // same-line adjacency between two bare names remains the ordinary
        // two-slot expression list, never a member edge.
        var result = Parser.ParseSyntax("A B");
    ...
    [InlineData("A~ # note\nB")]
    [InlineData("A~# note\nB")]
    public void Parse_CommentAfterPostfixGraceRun_DoesNotBridgeTheLineToAMember(string source)
    {
        // Comments are semantically invisible for line decisions: a trailing
        // comment must not let the next line's identifier become the
        // omitted-dot member — the physical-line rule reads real token
        // positions, so `A~ # note` newline `B` stays a graced row and a
        // separate `B` row, exactly like the comment-free spelling.
        var result = Parser.ParseSyntax(source);
    ...
    [InlineData("5~B")]
    [InlineData("x.y~B")]
    public void Parse_OrphanTildeAfterCompoundExpression_StaysPrefixGraceAdjacency(string source)
    {
        // A tilde after a completed compound expression is not consumed as
        // postfix grace (only bare names are eligible), so it re-enters as
        // PREFIX grace on the following identifier: two adjacency slots, no
        // omitted-dot edge and no eligibility error. The dotted spellings
        // (`(x)~.B` etc.) keep their targeted one-name rejection.
        var result = Parser.ParseSyntax(source);
    ...
    [Fact]
    public void Parse_OmittedDotGracedEdge_InIndexSelector_IsTheWholeSelectorPrimary()
    {
        // The indexing selector consumes ONE primary, and the omitted-dot
        // edge is produced by the primary parser, so `x:a~t` selects with the
        // whole edge: Index(x, DotCall(Grace(a), t)). The dotted spellings
        // stay chain continuations of the enclosing index — `x:a~.t` is
        // DotCall(Index(x, Grace(a)), t), where no `a~.t` subexpression
        // exists at all — so this is a grammar-boundary difference, not a
        // divergence of the two spellings of one edge.
        var compact = Parser.ParseSyntax("x:a~t");
        Assert.False(compact.HasErrors);
        var index = Assert.IsType<Expr.Index>(Assert.Single(compact.Root.Output));
        var selectorEdge = Assert.IsType<Expr.DotCall>(index.Selector);
        Assert.Equal("t", selectorEdge.Name);
        Assert.IsType<Expr.Grace>(selectorEdge.Target);

        var dotted = Parser.ParseSyntax("x:a~.t");
        Assert.False(dotted.HasErrors);
        var chained = Assert.IsType<Expr.DotCall>(Assert.Single(dotted.Root.Output));
        Assert.Equal("t", chained.Name);
        Assert.IsType<Expr.Index>(chained.Target);
    }

    [Fact]
    public void Parse_OmittedDotGracedEdge_ThenOrphanTilde_StaysPrefixGraceSlot()
    {
        // After the edge completes, a following tilde-run is not attached to
        // any dot, so it is an ordinary prefix-grace slot — while the dotted
        // `A~B~.C` spelling keeps the one-name rejection on its second edge.
        var result = Parser.ParseSyntax("A~B~C");

        Assert.False(result.HasErrors);
        Assert.Equal(2, result.Root.Output.Count);
        Assert.IsType<Expr.DotCall>(result.Root.Output[0]);
        var grace = Assert.IsType<Expr.Grace>(result.Root.Output[1]);
        Assert.Equal(-1, grace.Weight);
