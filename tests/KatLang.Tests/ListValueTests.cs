...
public class ListValueTests
{
    ...
    public void Call_EmptyListSpread_SuppliesZeroArguments()
    {
        ...
        // rejects the call, and a single collecting parameter collects the
        // empty list.
    ...
    [Fact]
    public void Call_CollectingParameter_KeepsListAsOneSuppliedItem()
        // The unspread list is one supplied argument, so the collecting
        // parameter collects the nested one-element list [[1, 2]].
        => AssertEvalCounted("F(*items) = items\nA = [1, 2]\nF(A)", 1, ListValue(ListValue(Atom(1), Atom(2))));

    [Fact]
    public void Call_CollectingParameter_SpreadSuppliesElements()
    ...
    [InlineData("Inspect([1, 2])", "[[1, 2]]")]
    [InlineData("Inspect([1, 2]*)", "[1, 2]")]
    public void CollectingCapture_CollectsExactListOfSuppliedArguments(string call, string expected)
