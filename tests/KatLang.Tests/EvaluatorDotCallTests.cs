...
public class EvaluatorDotCallTests
{
    ...
    public void Eval_ParenthesizedSequenceSpread_VariadicCallArgumentIsOneSequenceValue()
    {
        ...
        // call supplies as ONE argument; the collector collects it as one item
        // (count 1), alone or beside another argument, and only the spread
        // `(A*, B)*` supplies its three items.
        var source = """
            A = 1, 2
            B = 3, 4
    ...
    public void Eval_UserDefinedVariadicDotCallReceiver_BindsLikeTheWrittenCall()
    {
        ...
        // `CountItems((1, 2))`, collected as one item (count 1) exactly like the
        // written call; only the spread `(1, 2)*.CountItems` = `CountItems(1, 2)`
        // supplies two items. Beside a written argument the receiver is one
        // collected item, and a list receiver is the same rule.
        var source = """
    ...
    {
        // The fluent spread receiver supplies the receiver's items as argument
        // items — `(1, 2)*.Mean` is `Mean(1, 2)` — so the collecting parameter
        // collects [1, 2] and the numeric body works. The parenthesized spread
        // `((1, 2)*)` is a CAPTURE receiver instead: ONE sequence value, collected
        // as one non-numeric item — exactly like a captured spread of a list
        // (`([1, 2]*)` is `(1, 2)`) and the bare list receiver.
        var source = """
    ...
    // satisfies arity by its item count, and a collecting parameter collects it
    // exactly as it collects the written argument — as ONE item, a sequence and
    // a list alike. Only the spread marker opens a receiver into items: the
    // fluent `Values*.F(args)` is `F(Values*, args)`. Lean: CoreTests dot-call
    // receiver guards (DotReceiverSegments, ExplicitValueOpening).

    [Fact]
    public void Eval_SequenceValueReceiver_LeadingFlatVariadic_IsOneCollectedItem()
    {
        // The receiver is the one leading argument of `NItems(Pair)`, collected as
        // ONE item (count 1) — for a named property, an inline group, a brace-block
        // receiver, and a list receiver alike; only the spread receiver supplies
        // the two items.
        var source = """
    ...
    public void Eval_SequenceValueReceiverSpread_FluentSpreadSuppliesSlots_CaptureIsOneValue()
    {
        ...
        // items are collected as [10, 20] (count 2). The parenthesized `(Pair*)`
        // is a CAPTURE receiver — ONE sequence value (10, 20) — so
        // `(Pair*).NItems` is `NItems((Pair*))`: one collected item (count 1),
        // alone or beside a second written argument.
    ...
    [Fact]
    public void Eval_SequenceValueReceiver_LeadingFlatVariadicWithSuffix_IsOneItemAfterSuffixAllocation()
    {
        // Two supplied arguments: the receiver and 99. The suffix binds 99 from
        // the back FIRST; the collector collects the receiver's one value exactly
        // (count 1).
        var source = """
    ...
    public void Eval_SequenceValueReceiverSpreadWithSuffix_FluentSpreadSuppliesSlots_CaptureIsOneValue()
    {
        ...
        // value, so `(Pair*).BeforeLastCount(99)` leaves that one item to the
        // collector: count 1.
    ...
    {
        // Canonical-call twin of the dot-call above: Pair is one argument, the
        // suffix takes 99, and the collector collects the one remaining item —
        // count 1, matching Pair.BeforeLastCount(99).
        var source = """
            BeforeLastCount(*values, last) = values.count
            Pair = (10, 20)
    ...
    {
        // Each dot-call agrees with its canonical-call twin: the receiver is the
        // one argument — a sequence value collected as one item (count 1) — the
        // fluent spread receiver supplies the two spread items (count 2), and the
        // capture of the spread is again ONE sequence value (count 1).
        var define = """
            NItems(*values) = values.count
            Values = 10, 20
    ...
    public void Eval_MultiOutputReceiverWithSuffix_DotCallMatchesCanonicalCalls()
    {
        ...
        // collector collects the one remaining item (count 1); the fluent spread
        // forms supply the two spread items before the suffix (count 2); the
        // capture of the spread is ONE sequence value again (count 1).
    ...
    {
        // The named property receiver is a value boundary supplying ONE argument:
        // the collector collects it as one item (count 1) — only `Seq*.Sum()`
        // supplies three — and a fixed-arity callee sees exactly ONE argument.
        AssertEval(
            """
            Seq = (1, 2, 3)
