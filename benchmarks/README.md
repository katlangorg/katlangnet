...
- `EvaluatorIdentityStateBenchmarks` measures the evaluator's declaration / activation / scope-owner bookkeeping (invalid-state audit §3.1, September 2026) on eight prepared shapes — a parameterized call, three nested calls, an exported property cache hit, a local-only property read, `M.A`, a captured-ancestor read from a nested call, recursion to depth 40, and a clause-family call — with per-run time, allocated bytes, and Gen0 pressure; divide by `UnitsPerRun` for per-operation numbers (201 normally, 804 calls for the nested shape, 41 calls for recursion). Sources live in `EvaluatorIdentityStateScenarios` (the same class a paired Stopwatch harness can compile in). Filter: `--filter *EvaluatorIdentityState*`. `ParameterActivationCaptureBenchmarks` separately checks narrow-to-wide signature scaling (`--filter *ParameterActivationCapture*`); the wide path must deduplicate in linear time.
...
| Nested property chains | `benchmarks/KatLang.Benchmarks/Scenarios/nested-property-chains.kat` | Repeated one-hop receiver property lookup across nested algorithm contexts; the scenario deliberately stays one hop, although dot chains of any depth are legal KatLang. | `EvaluatorTests.Eval_Distinguishes_HigherOrderAlgorithmContexts` |
...
- `NestedPropertyChains` intentionally measures one-hop dot access. Multi-hop dot chains (`Lib.Sub.Q`, navigated structurally at every level) are legal KatLang; to measure them, add a new scenario instead of mutating this baseline.
