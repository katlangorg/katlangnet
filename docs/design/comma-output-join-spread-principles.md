...
11. Square brackets materialize an expression list as one EXACT immutable list value — a second materialization form beside principle 3's parentheses, with the opposite canonicalization contract: no singleton or empty erasure ever applies to list structure (`[7] != 7`, `[[]] != []`, `[] != ()`), while ordinary parentheses around a list stay redundant sequence grouping (`([1, 2]) == [1, 2]`). Element slots use the ordinary expression-list model including spread (`[0, A..., 4]`). Spread (principle 7) opens exactly one boundary of either collection kind (`[1, 2, 3]...` supplies three items; `[]...` supplies zero). Item-supply binding (principle 8) does NOT extend to lists: singleton-boundary normalization opens only grouped SEQUENCE values, a list stays one opaque supplied item at every call boundary, and builtin collection binding reports a targeted deferred-support error for list items. The lone-structure opening of assignment deconstruction is the one receiver that treats a lone list like a lone sequence (`x, y, z = [1, 2, 3]` opens), and its rest captures stay sequence-shaped. `[` always begins a new expression (never a call/indexing delimiter), so `A[1]` is the adjacency list `A, [1]`.
