...
all 2,286 surface cases is in the machine-readable report
...
so `[7].map(Collect)` collects `items = [7]`. Superseded once more in September 2026 by
VALUES STAY VALUES: a collector never opens an argument, so `F(V)` collects `[V]` for a
sequence and a list alike and never coincides with `F(V*)`
(`variadic_collect_distinguishes_spread_seq` / `_list`,
`receivers_never_same_on_lone_structure`), and every callback element is one ordinary
argument bound exactly as the direct call — the flat-callback row convention is gone.)*
