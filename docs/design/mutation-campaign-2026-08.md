...
| G14 | `ReCountValueBoundary` versus dot-receiver segment at the prepared-argument boundary (`Evaluator:2247`; both directions survive) | **IMPLEMENTED**, shared sync/async boundary helper plus source-level and direct tests; consolidated rerun pending. *(September 2026: the dot-receiver segment no longer exists — dot-call passes the receiver as an ordinary argument, so the boundary helper now serves written slots only.)* |
...
| Budgets | G13, G14 | Resource-limit retention and prepared-argument boundary counting are named shared policies, used by both sync and async evaluation. The August policy tests distinguished resource errors and the former dot-receiver segments. The receiver discriminator and its helper were superseded in September 2026: every non-spread receiver is now an ordinary argument; source-level patterned receiver tests pin equivalence with written calls. |
