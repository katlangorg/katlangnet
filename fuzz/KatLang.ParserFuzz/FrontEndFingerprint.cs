...
/// algorithms, operands, list elements, ...) and never follows the parent/back-reference
...
internal static class FrontEndFingerprint
{
    ...
    private static void ParamDecl(StringBuilder sb, ParameterDeclaration p)
        => sb.Append(p.Name).Append(':').Append(p.Kind).Append('@').Append(Span(p.Span)).Append(';');
    ...
    private static void Pattern(StringBuilder sb, Pattern p)
    {
        ...
        {
            case Pattern.Bind b:
                sb.Append("Bind{").Append(b.Name).Append(':').Append(b.ParameterKind).Append('@').Append(Span(b.NameSpan)).Append('}');
                break;
    ...
    private static void Expr(StringBuilder sb, Expr e)
    {
        ...
        switch (e)
        {
            ...
            case Expr.Block(var alg): sb.Append("Blk("); Alg(sb, alg); sb.Append(')'); break;
            case Expr.Call(var fn, var args): sb.Append("Call("); Expr(sb, fn); sb.Append(','); Alg(sb, args); sb.Append(')'); break;
            ...
            case Expr.DotCall dc:
                ...
                Expr(sb, dc.Target);
                if (dc.Args is not null) { sb.Append(",args="); Alg(sb, dc.Args); }
                sb.Append(')');
                break;
            default:
