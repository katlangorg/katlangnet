...
/// <summary>
/// Deterministic replay for the UTF-16 target.
///
/// <code>
/// utf16-replay PATHS...              replay every seed in the given manifest files/directories
/// utf16-replay --payload HEX ...     replay one or more ad-hoc payloads
/// utf16-replay --raw PATHS...        replay files whose CONTENT is the payload — the form
///                                    libFuzzer writes crash and corpus artifacts in
/// utf16-seeds OUTDIR [PATHS...]      write each seed's raw payload as a libFuzzer corpus file
/// </code>
///
/// <para>Replay runs the SAME decoder, builder, executor and relations as the fuzzing loop, and
/// runs every case TWICE — a non-deterministic observation is itself a replay failure. Every case
/// is reported with its exact code units in four-digit hex, which is the only lossless printable
/// form of possibly ill-formed UTF-16.</para>
///
/// <para>Exit code 0 iff every case was read, executed without an invariant violation, and replayed
/// identically. Replaying nothing is a failure, not a clean run.</para>
/// </summary>
internal static class Utf16Replay
{
    public static int RunReplay(string[] args)
    {
        ...
        if (args.Length < 2)
        {
            Console.Error.WriteLine("usage: utf16-replay [--raw] (SEED_MANIFEST | DIRECTORY | --payload HEX)...");
            return 2;
        }
        ...
        {
            Console.Error.WriteLine("utf16-replay: the given paths contain no seeds; nothing was verified.");
            return 2;
        }
        ...
        foreach (var seed in seeds)
        {
            ...
            try
            {
                first = Utf16Invariants.Run(seed.Payload, ref phase);
            }
            ...
            var templateId = Utf16Tables.TemplateOf(first.Case.Parameters.Template).Id;
            templates[templateId] = templates.GetValueOrDefault(templateId) + 1;
            var surrogateClass = first.Case.SurrogateClass.ToString();
            surrogateClasses[surrogateClass] = surrogateClasses.GetValueOrDefault(surrogateClass) + 1;
            var lineEnding = first.Case.Parameters.LineEndings.ToString();
            lineEndings[lineEnding] = lineEndings.GetValueOrDefault(lineEnding) + 1;
            fingerprints.Add(first.Fingerprint);

            Console.WriteLine(
                $"{seed.Location,-28} {Hex(seed.Payload),-34} units={first.Case.HexUnits}");
        ...
        Console.WriteLine(
            $"utf16-replay: {Num(seeds.Count)} case(s), {Num(failures)} failure(s), " +
            $"{Num(nondeterministic)} nondeterministic, {Num(fingerprints.Count)} distinct fingerprint(s).");
        Report("templates", templates);
        Report("surrogate classes", surrogateClasses);
        Report("line endings", lineEndings);
    ...
    public static int RunExportSeeds(string[] args)
    {
        ...
        if (args.Length < 2)
        {
            Console.Error.WriteLine("usage: utf16-seeds OUTDIR [SEED_MANIFEST | DIRECTORY]...");
            return 2;
        }
        ...
        var sources = args.Length > 2
            ? args.Skip(2)
            : [Path.Combine(AppContext.BaseDirectory, "Utf16Testcases")];
        ...
        if (seeds.Count == 0)
        {
            Console.Error.WriteLine("utf16-seeds: no seeds found; refusing to write an empty corpus.");
            return 2;
        }
        ...
        foreach (var seed in seeds)
        {
            var name = $"seed-{index.ToString("D3", CultureInfo.InvariantCulture)}-" +
                       $"{Utf16Tables.TemplateOf(seed.DeclaredTemplate).Id}";
            File.WriteAllBytes(Path.Combine(outDir, name), seed.Payload);
            index++;
        }

        Console.WriteLine($"utf16-seeds: wrote {Num(index)} payload(s) to {outDir}");
        return 0;
    }

    private static List<Utf16Seed> CollectSeeds(IEnumerable<string> args, List<string> problems)
    {
        ...
        foreach (var arg in args)
        {
            ...
            if (pendingPayloads)
            {
                if (Utf16SeedFile.TryParseHexBytes(arg, out var payload, out var problem))
                    seeds.Add(new Utf16Seed("--payload", seeds.Count + 1, Utf16Decoder.Decode(payload).Template, payload, "ad-hoc"));
                else
    ...
    private static void LoadOne(string file, bool raw, List<Utf16Seed> seeds, List<string> problems)
    {
        if (raw)
        {
            byte[] payload;
            ...
            // A raw artifact IS the payload — including the empty one, which is a valid case.
            var template = Utf16Decoder.Decode(payload).Template;
            seeds.Add(new Utf16Seed(Path.GetFileName(file), 1, template, payload, "raw artifact"));
            return;
        }
