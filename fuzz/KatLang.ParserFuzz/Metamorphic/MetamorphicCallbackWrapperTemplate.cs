...
/// <list type="bullet">
///   <item><b>Rest</b> — <c>MmWrap(xs...)</c> COLLECTS the supplied slots into an exact list, so
///   the wrapper sees <c>[element]</c> where the builtin sees <c>element</c>. Measured:
///   <c>[[1, 2], [3]].map(count)</c> is <c>[2, 1]</c> while the rest wrapper gives
...
internal static class MetamorphicCallbackWrapperTemplate
{
    ...
    internal static MetamorphicPrecondition Validate(MetamorphicParameters parameters)
    {
        ...
        {
            // A rest parameter COLLECTS the supplied slots into a list, so the wrapper receives
            // [element] where the direct builtin receives element. Never equivalent.
            MetamorphicWrapperProjection.Rest =>
                MetamorphicPrecondition.Rejected("rest-projection-collects-a-list-not-the-supplied-value"),
    ...
    private static string WrapperDefinition(MetamorphicBuiltin callback, int arity, MetamorphicWrapperProjection projection)
    {
        ...
        switch (projection)
        {
            ...
            case MetamorphicWrapperProjection.Rest:
